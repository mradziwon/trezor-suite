...
class RippleWorker extends BaseWorker<RippleAPI> {
    ...
    async connect(): Promise<RippleAPI> {
        ...
        // validate endpoints
        const { server, timeout } = this.state.getSettings();
        if (!server || !Array.isArray(server) || server.length < 1) {
            throw new CustomError('connect', 'Endpoint not set');
        }

        if (this.endpoints.length < 1) {
            this.endpoints = this.shuffleEndpoints(server.slice(0));
        }

        this.state.debug('Connecting to', this.endpoints[0]);
        ...
        try {
            api = new RippleAPI({
                ...
                connectionTimeout: timeout || DEFAULT_TIMEOUT,
            });
            ...
            this.state.debug('Websocket connection failed', error);
            this.api = undefined;
    ...
    setPingTimeout() {
        ...
        this.pingTimeout = setTimeout(
            () => this.onPing(),
            this.state.getSettings().pingTimeout || DEFAULT_PING_TIMEOUT
        );
    }
    ...
    async onPing() {
        ...
        if (this.state.hasSubscriptions() || this.state.getSettings().keepAlive) {
            try {
                await this.api.getServerInfo();
            } catch (error) {
                this.state.debug(`Error in timeout ping request: ${error}`);
            }
            // reset timeout
            this.setPingTimeout();
        } else {
